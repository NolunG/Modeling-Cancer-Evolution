% All of the variable follow names from the report
alphar= 0.20;
betar= 1e-10;
alphas= 0.20;
betas= 1e-10;
...
t2 = 842;
t3=1131;
p = 21; %Days differene in treatment
q = 3;  % Number of treatments
...
for i=2:iteration
    ...
    if i == 2335
     ...
     if((mod(i-t1,p)==0 && ((i-t1)/p <= (CycleLen-1)) && i>=t1) || ((mod(i-t2,p)==0 && ((i-t2)/p <= (CycleLen-1)) && i>=t2)))% || ((mod(i-t3,p)==0 && ((i-t3)/p <= (CycleLen-1)) && i>=t3)))
